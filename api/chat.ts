...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  if (!messages || !Array.isArray(messages)) {
    ...
    res.end(JSON.stringify({ error: "messages array is required" })); return;
  }

  const apiKey = process.env.CEREBRAS_API_KEY;
  if (!apiKey) {
    res.writeHead(500, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ error: "Cerebras API key not configured" })); return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      res.end(JSON.stringify({ error: "AI service error" })); return;
    }

    const data = (await response.json()) as { choices: { message: { content: string } }[] };
    const raw = data.choices?.[0]?.message?.content ?? "{}";

    let parsed: { reasoning?: unknown[]; message?: string; sources?: unknown[] } = {};
    try { parsed = JSON.parse(raw); } catch { parsed = { message: raw, reasoning: [], sources: [] }; }
